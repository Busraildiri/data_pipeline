...
DB_HOST = os.environ.get("TAYNA_DB_HOST")
DB_PORT = int(os.environ.get("TAYNA_DB_PORT", "3306"))
DB_USER = os.environ.get("TAYNA_DB_USER")
DB_PASSWORD = os.environ.get("TAYNA_DB_PASSWORD")
DB_NAME = os.environ.get("TAYNA_DB_NAME", "defaultdb")



def get_engine():
    missing = [name for name, value in {
        "TAYNA_DB_HOST": DB_HOST,
        "TAYNA_DB_USER": DB_USER,
        "TAYNA_DB_PASSWORD": DB_PASSWORD,
    }.items() if not value]
    if missing:
        raise ValueError(f"Eksik MySQL ortam değişkenleri: {', '.join(missing)}")

    connection_url = URL.create(
        "mysql+pymysql", username=DB_USER, password=DB_PASSWORD,
        host=DB_HOST, port=DB_PORT, database=DB_NAME,
    )
    return create_engine(connection_url, connect_args={"ssl": {}}, pool_pre_ping=True)
